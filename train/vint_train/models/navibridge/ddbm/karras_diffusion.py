...
class KarrasDenoiser:
    ...
    def get_weightings(self, sigma):
        ...
        weightings = th.where(th.isfinite(weightings), weightings, th.zeros_like(weightings))
        return weightings

    def get_bridge_scalings(self, sigma):
        if self.pred_mode == 've':
            A = sigma**4 / self.sigma_max**4 * self.sigma_data_end**2 + \
                (1 - sigma**2 / self.sigma_max**2)**2 * self.sigma_data**2 + \
                2*sigma**2 / self.sigma_max**2 * (1 - sigma**2 / self.sigma_max**2) * self.cov_xy + \
                self.c **2 * sigma**2 * (1 - sigma**2 / self.sigma_max**2)
            c_in = 1 / (A) ** 0.5
            c_skip = ((1 - sigma**2 / self.sigma_max**2) * self.sigma_data**2 + sigma**2 / self.sigma_max**2 * self.cov_xy) / A
            c_out = ((sigma/self.sigma_max)**4 * (self.sigma_data_end**2 * self.sigma_data**2 - self.cov_xy**2) + \
                    self.sigma_data**2 *  self.c **2 * sigma**2 * (1 - sigma**2/self.sigma_max**2) )**0.5 * c_in
            ...
            A = a_t**2 * self.sigma_data_end**2 + b_t**2 * self.sigma_data**2 + \
                2*a_t * b_t * self.cov_xy + self.c**2 * c_t
            
            c_in = 1 / (A) ** 0.5
            c_skip = (b_t * self.sigma_data**2 + a_t * self.cov_xy) / A
            c_out = (a_t**2 * (self.sigma_data_end**2 * self.sigma_data**2 - self.cov_xy**2) + \
                    self.sigma_data**2 *  self.c **2 * c_t )**0.5 * c_in
            return c_skip, c_out, c_in
    ...
    def training_bridge_losses(self, model, x_start, sigmas, global_cond=None, model_kwargs=None, noise=None, vae=None, train=True):
        ...
        model_output, denoised = self.denoise(model, x_t, sigmas, global_cond, **model_kwargs)
        ...
        return terms, denoised

    def denoise(self, model, x_t, sigmas, global_cond=None, **model_kwargs):
        c_skip, c_out, c_in = [
        ...
        model_output = model("noise_pred_net",
                             sample=c_in * x_t,
                             timestep=rescaled_t,
...
def karras_sample(
    ...
    def denoiser(x_t, sigma, x_T=None, global_cond=None):
        _, denoised = diffusion.denoise(model, x_t, sigma, global_cond)
        if clip_denoised:
...
def sample_heun(
    ...
    for j, i in enumerate(indices):
        if churn_step_ratio > 0:
            ...
            denoised = denoiser(x, sigmas[i] * s_in, x_T, global_cond)
            if pred_mode == 've':
        ...
        denoised = denoiser(x, sigma_hat * s_in, x_T, global_cond)
        if pred_mode == 've':
        ...
        if sigmas[i + 1] == 0:
            ...
            x_2 = x + d * dt
            denoised_2 = denoiser(x_2, sigmas[i + 1] * s_in, x_T, global_cond)
            if pred_mode == 've':
                d_2 = to_d(x_2, sigmas[i + 1], denoised_2, x_T, sigma_max, w=guidance)
            elif pred_mode.startswith('vp'):
                d_2 = get_d_vp(x_2, denoised_2, x_T, std(sigmas[i + 1]), logsnr(sigmas[i + 1]), logsnr_T, logs(sigmas[i + 1]), logs_T, s_deriv(sigmas[i + 1]), vp_snr_sqrt_reciprocal(sigmas[i + 1]), vp_snr_sqrt_reciprocal_deriv(sigmas[i + 1]), guidance)
